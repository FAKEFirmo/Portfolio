...
Static original artwork in `public/images/amber.webp`. The hero opens with one orchestrated load sequence and the artwork drifts against the scroll where the browser supports scroll-driven timelines. A warm light rises along the sculpture's lit edges as the hero scrolls past, masked by the artwork's own luminance so it reaches the highlights and leaves the dark background untouched. Nothing on the page animates on a clock: every ambient effect is driven by scroll position, so once the entrance sequence finishes the page produces no frames until the reader moves. The artwork is never scaled, because animating a viewport-sized bitmap costs a re-rasterisation every frame. Section entrance effects, glass highlights and hover transitions respect reduced-motion preferences, and every animated state has the resting state as its base, so the page reads correctly with motion disabled or JavaScript unavailable. The glass treatment uses layered highlights, saturation and a modest backdrop blur with a readable dark fallback. An SVG refraction filter was tried on the backdrop and removed: `feTurbulence` plus `feDisplacementMap` has to regenerate whenever the backdrop moves, which cost more than the effect was worth at these panel sizes. The reference liquid-glass-js library was reviewed; its continuous WebGL/page sampling is intentionally not a dependency of this static first phase.
